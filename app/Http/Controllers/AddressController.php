...
//        return response()->json('good');
        ...
        foreach ($user->addresses as $address_) {
            $region_city = [];
            if($address_->cities){
                $city_translate = table_translate_title($address_->cities,'city',$language);
                if($address_->cities->type == 'district'){
                    ...
                    if($address_->cities->region){
                        $region_translate = table_translate_title($address_->cities->region,'city',$language);
                        $region = [
                        ...
                        if(!$address_->cities->region->getDistricts->isEmpty()){
                            foreach($address_->cities->region->getDistricts as $regionCity){
                                $region_city_translate = table_translate_title($regionCity,'city',$language);
                                $region_city[] = [
                                    'id' => $regionCity->id,
    ...
    public function destroy(Request $request){
        ...
        if($address){
            if($address->order){
                return $this->success(translate_title('prohibited'), 200);
            }
            $address->delete();
            return $this->success('Success', 200);
        }else{
